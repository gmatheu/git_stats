...
")
      ...
      it 'should parse git revlist output to date sorted commits array' do
        repo.should_receive(:run).with("git rev-list --pretty=format:'%h|%at|%ai|%aE|%s' HEAD . | grep -ve '^commit'").and_return(
            "e4412c3|1348603824|2012-09-25 22:10:24 +0200|john.doe@gmail.com|message
ce34874|1347482927|2012-09-12 22:48:47 +0200|joe.doe@gmail.com|second message
5eab339|1345835073|2012-08-24 21:04:33 +0200|john.doe@gmail.com|third and long message
")

        repo.commits.should == [
            GitStats::GitData::Commit.new(
                repo: repo, sha: "5eab339", stamp: "1345835073", date: DateTime.parse("2012-08-24 21:04:33 +0200"),
                author: repo.authors.first! { |a| a.email == "john.doe@gmail.com" }, message: 'message'),
            GitStats::GitData::Commit.new(
                repo: repo, sha: "ce34874", stamp: "1347482927", date: DateTime.parse("2012-09-12 22:48:47 +0200"),
                author: repo.authors.first! { |a| a.email == "joe.doe@gmail.com" }, message: 'second message'),
            GitStats::GitData::Commit.new(
                repo: repo, sha: "e4412c3", stamp: "1348603824", date: DateTime.parse("2012-09-25 22:10:24 +0200"),
                author: repo.authors.first! { |a| a.email == "john.doe@gmail.com" }, message: 'third and long message')
        ]
      end
    end
